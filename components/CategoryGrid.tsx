...
const CategoryGrid = async () => {
  ...
  let unisexCategory = null

  try {
    const categories = await getAllCategories()
    
    // Find men, women, and unisex categories by slug
    ...
    unisexCategory = categoriesArray.find(cat => cat.slug?.current === 'unisex')
  } catch (error) {
  ...
  return (
    <div className="w-full py-12 md:py-16 lg:py-20">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        {/* UNISEX Card */}
        <Link href={`/category/${unisexCategory?.slug?.current || 'unisex'}`}>
          <div className="relative w-full aspect-[3/4] overflow-hidden group cursor-pointer">
            {/* Image from Sanity */}
            {unisexCategory?.image?.asset?.url ? (
              <Image
                src={unisexCategory.image.asset.url}
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/80 to-transparent p-4 md:p-6 pt-12 md:pt-16">
              <h3 className="text-white font-semibold text-3xl md:text-3xl uppercase lg:text-4xl xl:text-5xl ransition-colors duration-300">
                {unisexCategory?.title || "Unisex Collection"}
              </h3>
              <p className="text-white/80 text-sm md:text-base group-hover:text-white transition-colors duration-300">
                {unisexCategory?.description || 'Explore our versatile collection'}
              </p>
            </div>
          </div>
        </Link>
